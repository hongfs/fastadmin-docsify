...
namespace Hongfs\FastadminDocsify;
...
class Generate extends Command
{
    protected function configure()
    {
        ...
        $this
            ...
            ->addOption('mdoutput', 'mo', Option::VALUE_OPTIONAL, 'output index file name', 'api.md')
            ->addOption('template', 'e', Option::VALUE_OPTIONAL, '', 'index.html')
            ->addOption('mdtemplate', 'me', Option::VALUE_OPTIONAL, '', 'index.md')
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        $mdtemplate = $input->getOption('mdtemplate');
        if (!preg_match("/^([a-z0-9]+)\.html\$/i", $template)) {
            throw new Exception('template file not correct');
        }
        if (!preg_match("/^([a-z0-9]+)\.md\$/i", $mdtemplate)) {
            throw new Exception('mdtemplate file not correct');
        }
        // 目标目录
        ...
        $mdoutput_file = $output_dir . $input->getOption('mdoutput');
        if (is_file($mdoutput_file) && !$force) {
            throw new Exception("api index file already exists!\nIf you need to rebuild again, use the parameter --force=true ");
        }
        // 模板文件
        ...
        $mdtemplate_file = $template_dir . $mdtemplate;
        if (!is_file($mdtemplate_file)) {
            throw new Exception('mdtemplate file not found');
        }
        // 额外的类
		...
		$data = $view->display(file_get_contents($mdtemplate_file), [
            'docsList' => $builder->parse(),
        ]);

        if (!file_put_contents($mdoutput_file, $data)) {
            throw new Exception('Cannot save the content to ' . $mdoutput_file);
        }

		$data = $view->display(file_get_contents($template_file), [
			'config' => $config,
		]);
